...
var articleImg = multer.diskStorage({
    destination: function (req, file, cb) { // 设置文件存储路径
        cb(null, './upload/image/articleImg')
    },
    filename: function (req, file, cb) { // 设置文件名字格式
        if (file) {
            cb(null, "upload_" + Date.parse(new Date()) + "_" + file.originalname);
...
var articleImgUpload = multer({
    storage: articleImg // 应用设置
});
...
router.post('/articleImg', User.signinRequired, articleImgUpload.single('articleImg'), Personal.UploadArticleImg);
